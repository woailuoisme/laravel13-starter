...
it('creates and verifies a sign in challenge when forced by risk policy', function (): void {
    $passSecret = implode('', ['pass', 'word123']);
    $user = User::factory()->create([
        'nickname' => 'unit_signin',
        'email' => 'unit-signin@example.com',
        'password' => Hash::make($passSecret),
    ...
    $requestResult = $service->requestSignin($user->email, $passSecret, '192.168.0.2', 'auto');
    ...
    expect($authenticatedUser->is($user))
        ->toBeTrue()
