...
it('exports users with simple excel', function (): void {
    ...
    expect($rows)->toHaveCount(2)
        ...
        ->and($rows[0]['openid'])
        ->toBe($olderUser->open_id)
        ->and($rows[0]['email'])
        ->toBe($olderUser->email)
        ->and($rows[1]['nickname'])
        ->toBe('unknown');
});
...
it('imports users with simple excel and upserts by email', function (): void {
    ...
    expect($result)->toBe(['imported' => 2, 'skipped' => 1])
        ...
        ->toBe('Updated')
        ->and(User::query()->where('email', 'existing@example.com')->value('open_id'))
        ->toBe('new_openid')
        ->and(User::query()->where('email', 'new@example.com')->exists())
        ->toBeTrue();
});
